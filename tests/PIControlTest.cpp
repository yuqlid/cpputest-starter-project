...
TEST(PI, test1) {
  ...
  outfile << "ref,input,input_clamp,output" << std::endl;
  ...
  for (size_t i = 0; i < period - 1; ++i) {
    ...
    output.at(i + 1) = out;
    outfile << static_cast<float>(i) / fs_hz << ", " << input << ", "
            << input_clamp << ", " << out << std::endl;
  }
